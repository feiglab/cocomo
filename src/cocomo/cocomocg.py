...
from typing import Dict, Iterable, Iterator, List, Optional, Tuple, Union, Set, Mapping
from types import MappingProxyType
from math import copysign, sqrt

from openmm import Vec3, System, Platform, LangevinIntegrator, XmlSerializer
from openmm import HarmonicBondForce, HarmonicAngleForce, CMMotionRemover, CustomNonbondedForce
from openmm.app import Topology, element, Simulation
...
EPS_V1 = { 'catpi_propro': eps_catpi_propro_v1,
           'catpi_prona':  eps_catpi_prona_v1,
           'pipi_propro':  eps_pipi_propro_v1 }

EPS_V2 = { 'catpi_propro': eps_catpi_propro_v2,
           'catpi_prona':  eps_catpi_prona_v2,
           'pipi_propro':  eps_pipi_propro_v2 }

# --- Other useful data -------------------------------------------------------

aminoacids = ['ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'GLY', 
              'HIS', 'ILE', 'LEU', 'LYS', 'MET', 'PHE', 'PRO', 'SER', 
              'THR', 'TRP', 'TYR', 'VAL', 'HSD', 'HSE']
...
class COCOMO:
    def __init__(self, structure: Structure | None = None, *, 
                 version=None, box=100, cuton=2.9, cutoff=3.1, switching='original',
                 kappa=1.0, removecmmotion=True, xml=None):
    ...
    def setup_simulation(self,*,temp=298, gamma=0.01, tstep=0.01, resources='CPU') -> None:
        ...
        if self.resources == 'CUDA':
            ...
            self.simulation=Simulation(self.topology,self.system,self.integrator,self.platform,prop)
    ...
    def set_positions(self,positions=None) -> None:
        ...
        if positions is not None:
            ...
            assert self.structure is not None, "need structure to be defined if not providing positions as argument"
    ...
    def setupBondForce(self, *, l0protein=bond_l0_protein, l0na=bond_l0_na, kprotein=bond_k_protein, kna=bond_k_na) -> None:
        if self.topology is not None:
            force = HarmonicBondForce()
            for bond in self.topology.bonds():
                if bond[0].residue.name in aminoacids:
                     force.addBond(bond[0].index, bond[1].index, l0protein*nanometer, kprotein*kilojoules_per_mole/(nanometer**2))
                elif bond[0].residue.name in nucleicacids:
                     force.addBond(bond[0].index, bond[1].index, l0na*nanometer, kna*kilojoules_per_mole/(nanometer**2))
                else:
                     raise ValueError(f'Invalid residue name {bond[0].residue.name} in setupBondForce')

            force.setName("bond")
            self.forces['bond']=force 
            self.system.addForce(force)

    def setupAngleForce(self, *, t0protein=angle_t0_protein, t0na=angle_t0_na, kprotein=angle_k_protein, kna=angle_k_na) -> None:
        if self.topology is not None:
            ...
            for chain in self.topology.chains():
                ...
                for i in range(len(chatm)-2):
                    if chatm[i].residue.name in aminoacids:
                        force.addAngle(chatm[i].index, chatm[i+1].index, chatm[i+2].index, 
                                       t0protein*degrees, kprotein*kilojoule_per_mole/(radian**2))
                    elif chatm[i].residue.name in nucleicacids:
                        force.addAngle(chatm[i].index, chatm[i+1].index, chatm[i+2].index, 
                                       t0na*degrees, kna*kilojoule_per_mole/(radian**2))
                    else:
                        raise ValueError(f'Invalid residue name {chatm[i].residue.name} in setupAngleForce')
    ...
    def setupShortRangeForce(self) -> None:
        if self.topology is not None:
            ...
            if self.switching == 'original':
                ...
                equation += "sdel = sk*((1/r_on)^3-1/(r_off)^3)^2 - 4*eps_eff*((sigma/r_on)^10-(sigma/r_on)^5); "
                equation += "switch = sk*((1/r)^3-1/(r_off)^3)^2; "
                equation += "sk = -shortrange_deriv_Ron/switch_deriv_Ron; "
                equation += "shortrange_deriv_Ron = 4*eps_eff*(-10*(sigma/r_on)^11*1/r_on+5*(sigma/r_on)^5*1/r_on ); "
                equation += "switch_deriv_Ron = 6*(1/r_on^3-1/r_off^3)*1/r_on^4;"
            else:
                equation = "4*eps_eff*((sigma/r)^10-(sigma/r)^5);" 
            equation += "sigma=0.5*(sigma1+sigma2);"
            equation += "eps_eff = eps_mix + eps_catpi_pro*min(1,catpipropair) + eps_catpi_na*min(1,catpinapair) + eps_pipi*min(1,catpipipair);"
